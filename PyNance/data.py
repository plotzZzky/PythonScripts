...
class GetData:
    ...
    def __init__(self):
        self.sobre_rendimentos = "Valores recebidos neste mês atraves de salarios, investimentos e passivos"
        self.sobre_gastos_essenciais = 'Gastos recorentes e essencias para sua vida economica.'
    ...
    def get_income(self):
        ...
        self.investimentos = self.get_value("Digite o valor liquido do lucro dos seus investimentos:\n")
        self.passivos = self.get_value("Digite o valor liquido da sua renda passiva:\n")
        self.outras_rendas = self.get_value("Digite o valor liquido recebido de outras formas de renda:\n")

        self.rendimentos = self.salario + self.investimentos + self.passivos + self.outras_rendas
    ...
    def get_essential_expenses(self):
        ...
        try:
            ...
            self.servicos = self.get_value("Digite o valor gasto com serviços de luz, agua, gas etc.:\n")
            self.alimentacao = self.get_value("Digite o valor gasto com alimentção do dia a dia, "
                                              "não inclua idas ocasionais a restaurantes e comidas especias:\n")
            self.saude = self.get_value("Digite o valor gasto com saude, medicos e farmacia:\n")
            self.educacao = self.get_value("Digite o valor gasto com educação:\n")

            self.gastos_essenciais = (self.telefonia + self.aluguel + self.transporte + self.saude +
    ...
    def get_personal_expenses(self):
        ...
        try:
            self.cartao = self.get_value("Digite o valor gasto com cartão de credito:\n")
            self.telefonia = self.get_value("Digite o valor gasto com telefonia e internet:\n")
            self.hobby = self.get_value("Digite o valor gasto com hobby:\n")
            self.viagens_e_passeios = self.get_value("Digite o valor gasto com viagens e passeios:\n")
            self.compras = self.get_value("Digite o valor gasto com compras não essenciais:\n")
            self.refeicoes_especiais = self.get_value(
                "Digite o valor gasto refeições especias como idas a restaurantes e outros:\n")

            self.gastos_pessoais = (self.cartao + self.telefonia + self.hobby + self.viagens_e_passeios
                                    + self.compras + self.refeicoes_especiais)
    ...
    def get_investments_and_savings(self):
        ...
        try:
            self.poupanca = self.get_value("Digite o valor destinado a poupança esse mes:\n")
            self.reserva = self.get_value("Digite o valor destinado a reserva de emergencia esse mes:\n")
            self.novos_investimentos = self.get_value("Digite o valor destinado a novos investimentos:\n")
    ...
    def get_percentagem(self):
        self.saldo = self.rendimentos - (self.gastos_essenciais + self.gastos_pessoais + self.economias)
        ...
        print(f"A porcentagem do gasto com aluguel nesse mês foi de {self.porcentagem_aluguel}%")

        # Porcentagem da receita destinados aos gastos essenciais 
        self.porcentagem_gastos_essenciais = self.calculate_percentagem(self.gastos_essenciais)
        print(f"A porcentagem dos gastos essenciais nesse mês foi de {self.porcentagem_gastos_essenciais}%")

        # Porcentagem da receita destinada aos gastos pessoais
        self.porcentagem_gastos_pessoais = self.calculate_percentagem(self.gastos_pessoais)
        print(f"A porcentagem dos gastos pessoais nesse mês foi de {self.porcentagem_gastos_pessoais}%")
    ...
    def test(self):
        ...
        self.educacao = 0
        self.gastos_essenciais = (self.aluguel + self.transporte + self.saude + self.servicos + self.alimentacao +
                                  self.educacao)
        ...
        self.gastos_pessoais = (self.telefonia + self.hobby + self.viagens_e_passeios + self.compras +
